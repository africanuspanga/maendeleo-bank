...
export default async function TendersPage() {
	...
	return (
		<>
			<PageHeader
				title="Tenders"
				description="Procurement notices and requests for proposals shown on the public tenders page."
				actions={
					<Button
						className="h-9 rounded-full bg-[#843b8d] text-white hover:bg-[#6f2f78]"
						render={<Link href="/admin/tenders/new" />}
					>
						<PlusIcon />
						New tender
					</Button>
									...
									<TableRow key={item.id}>
										...
										<TableCell>
											<RowActions
												id={item.id}
												status={item.status}
												editHref={`/admin/tenders/${item.id}`}
												onSetStatus={setTenderStatus}
